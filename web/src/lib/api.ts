...
export const api = {
  ...
  runAgent: (id: string, inputs: Record<string, string | number | boolean> = {}) =>
    j<{ run: AgentRunRecord }>(`/api/app/agents/${id}/runs`, { method: "POST", body: JSON.stringify({ inputs }) }).then((payload) => payload.run),
