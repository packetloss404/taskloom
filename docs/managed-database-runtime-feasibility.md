...
8. Keep unsupported managed database hints, missing adapter/URL pairs, multi-writer/distributed topology, regional failover, PITR, and active-active writes blocked until a real topology package documents the owner, consistency model, failover/PITR plan, migration/backfill plan, observability requirements, rollback strategy, review outcome, approvers, authorization scope, and approval timing.
