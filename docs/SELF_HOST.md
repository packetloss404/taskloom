...
You are now in the workbench. Go to `/builder`, choose **Build an app**, and try a starter prompt like `Build a lightweight CRM for renewal tracking`.
...
1. **Per-workspace in the workbench** (recommended for most users). Open the **Providers** view in the workbench. Paste your key. It is stored in the encrypted secrets vault (AES-256-GCM at rest), never logged, never sent anywhere except to the LLM provider you selected.
...
Get a key at https://console.anthropic.com. The default model Taskloom targets is `claude-sonnet-4-6` — a good balance of cost, latency, and quality for builder workloads. For more demanding multi-turn agent work, switch per-agent to `claude-opus-4-7` from the Providers view.
...
Get a key at https://platform.openai.com/api-keys. Default model is `gpt-4o`; switch per-agent in the Providers view.

### Option C — Local Ollama (no cloud LLM)
...
# or, for code-specialized work:
ollama pull qwen2.5-coder:32b
```

Smaller Ollama models produce noticeably worse generated apps than Claude or GPT-4-class hosted models — this is honest, not pessimistic. If you need fully offline and don't want to compromise quality, plan to run `qwen2.5-coder:32b` or larger on a machine with 32 GB+ RAM and a recent GPU.
...
### MiniMax (optional, additional provider)
...
Configured the same way as Anthropic / OpenAI. Used by some agent templates that want a non-Anthropic, non-OpenAI option.
...
The builder fell back to template-only generation because it could not find an LLM key. Check, in order:

1. Open the **Providers** view in the workbench. Is a key configured for the provider you selected (Anthropic / OpenAI / Ollama / MiniMax)?
...
- Read [BACKLOG.md](../BACKLOG.md) for the in-flight self-host improvements.
