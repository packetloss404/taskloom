...
- [ ] Continue the relational-repository migration started in Phases 32-35 (`jobMetricSnapshots`, `alertEvents`, `agentRuns`, `jobs`) through queued collections: `invitationEmailDeliveries`, `activities`, then drop the legacy JSON-side mirrors and flip the deferred scheduler hot-path to the repository's SQL-native `claimNext`/`sweepStaleRunning` primitives. See `docs/roadmap-relational-repositories.md`.
