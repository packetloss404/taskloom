...
- Dedicated relational repository for `activities`: SQLite migration `0015_activities.sql` plus `src/repositories/activities-repo.ts`. `listActivitiesForWorkspaceIndexed(workspaceId, limit?)` delegates through the repository while activity writes dual-write to both `app_records` and the dedicated table in SQLite mode. Operator CLIs `db:backfill-activities` and `db:verify-activities` cover backfill and drift detection.
...
This phase does not change the JSON-default runtime, remove the collection fields from `TaskloomData`, remove the backfill/verify CLIs, or flip the deferred Phase 35 scheduler hot path to the repository SQL-native `claimNext`/`sweepStaleRunning` methods.
...
- [ ] Flip the deferred scheduler hot-path to the repository's SQL-native `claimNext`/`sweepStaleRunning` primitives once the relevant verify commands report zero drift.
