...
- `docs/deployment-release-readiness.md` documents the Phase 43 deployment release-readiness gate exposed through `npm run deployment:check-release` and `npm run deployment:check-release -- --strict`. The gate checks backup/restore readiness, persistent paths, and storage topology, but it does not perform backups, restore data, provision managed DBs, or make SQLite distributed.
- `docs/deployment-release-evidence.md` documents the Phase 44 release evidence bundle export exposed through `npm run deployment:export-evidence` and `npm run deployment:export-evidence -- --strict`. The export packages readiness, topology, and environment evidence for handoff, redacts secrets, and does not run backups, tests, or builds; restore data; provision managed DBs; or make SQLite distributed.
...
- The gate checks backup/restore readiness, persistent SQLite/artifact/log paths, and Phase 42 storage topology findings before production handoff.
...
- The export packages readiness, topology, persistent-path, backup/restore-readiness, and environment/runtime evidence for release handoff.
...
- [ ] Continue production hardening implementation beyond process/store-scoped rate limiting, webhook email delivery, CSRF checks, app-level redaction, Phase 20 export/access-log controls, Phase 21 scheduler leader election, Phase 22 managed external email provider operations, Phase 23 access-log shipping/retention, Phase 24 operational status/health endpoints, Phase 42 storage topology readiness reporting, Phase 43 release-readiness gates, Phase 44 release evidence exports, Phase 45 managed database topology advisory reporting, and Phase 46 runtime guardrails, such as actual managed production database topology support and dedicated relational repositories where indexed `app_records` metadata is not enough. SQLite/database topology guidance lives in `docs/deployment-sqlite-topology.md`, the Phase 42 advisory workflow lives in `docs/deployment-storage-topology.md`, the Phase 43 release gate lives in `docs/deployment-release-readiness.md`, the Phase 44 evidence bundle lives in `docs/deployment-release-evidence.md`, the Phase 45 managed database advisory lives in `docs/deployment-managed-database-topology.md`, and the Phase 46 runtime guard lives in `docs/deployment-managed-database-runtime-guard.md`.
...
1. Production storage implementation hardening beyond the landed Phase 42 advisory report, Phase 43 release-readiness gate, Phase 44 release evidence export, Phase 45 managed database topology advisory, and Phase 46 runtime guardrails: actual managed database runtime support and dedicated relational repositories/backfills where indexed `app_records` metadata is not enough.
2. Continued workflow/UI, test, and release hardening.

## Near-Term Definition Of Done

The near-term production hardening track is complete when the Phases 20 through 46 deployment, observability, alerting, workflow-polish, relational-repository, storage-readiness, release-readiness, release-evidence, managed-database-advisory, and runtime-guard milestones are paired with actual managed database topology where needed, and the existing route-level RBAC/workflow/job/agent/share parity suite continues to pass on both storage modes. The relational-repository track now includes Phase 38 mirror retirement for the first six migrated collections, the no-migration jobs scheduler hot-path follow-up, the Phase 39 `providerCalls` cutover and mirror retirement, and the Phase 40 `activationSignals` dedicated-table cutover plus post-Phase-40 mirror retirement. Phase 42 adds the advisory readiness report through `npm run deployment:check-storage`, admin API, and Operations UI, Phase 43 layers `npm run deployment:check-release` and strict mode over backup/restore readiness, persistent paths, and storage topology, Phase 44 packages redacted handoff evidence through `npm run deployment:export-evidence`, Phase 45 adds `npm run deployment:check-managed-db` with strict mode for managed database topology advisory reporting, and Phase 46 adds `npm run deployment:check-runtime-guard` with strict mode for blocking unsupported managed database/runtime hints before startup or release automation; none of those phases closes the managed database implementation gap. Remaining gaps are managed database runtime support and continued workflow/UI/test polish for features not yet covered; SQLite remains a local single-node posture, not a multi-writer or distributed production topology.
