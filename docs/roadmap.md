...
1. Keep Phase 52 managed/Postgres startup assertion validation wired into release-readiness, evidence, topology advisory, and runtime-guard checks so only recognized Postgres adapter + managed database URL configurations are accepted.
2. Keep Phase 53 as the active requirements/design gate for horizontal database writers, regional failover, PITR, and active-active writes. These requirements should block release/handoff claims until a real topology is designed and owned.
3. Keep Phase 54 as the owned design-package gate before implementation planning. The package must document owner, consistency model, failover/PITR plan, migration/backfill plan, observability requirements, and rollback strategy.
4. Keep Phase 55 as the review/implementation-authorization evidence gate before runtime implementation starts. The evidence must document the reviewed package version, review outcome, approvers, authorization scope, approval timing, and release-evidence attachment.
5. Keep Phase 56 as the implementation-readiness/rollout-safety evidence gate before runtime support can be claimed. The evidence must document implementers, work items, test and migration dry-run proof, staged rollout controls, rollback/recovery proof, observability proof, and release-claim boundaries.
6. Keep Phase 57 as the implementation-scope gate before implementation claims can proceed. The evidence must document Phase 56 dependency, scope lock, runtime feature flag/deployment gate, validation evidence, migration/cutover lock, and release-owner signoff.
7. Keep Phase 58 as the runtime-implementation validation gate after Phase 57. The evidence must document `TASKLOOM_MULTI_WRITER_RUNTIME_IMPLEMENTATION_EVIDENCE`, `TASKLOOM_MULTI_WRITER_CONSISTENCY_VALIDATION_EVIDENCE`, `TASKLOOM_MULTI_WRITER_FAILOVER_VALIDATION_EVIDENCE`, `TASKLOOM_MULTI_WRITER_DATA_INTEGRITY_VALIDATION_EVIDENCE`, `TASKLOOM_MULTI_WRITER_OPERATIONS_RUNBOOK`, and `TASKLOOM_MULTI_WRITER_RUNTIME_RELEASE_SIGNOFF`.
8. Keep Phase 59 as the release-enable approval gate after Phase 58. The evidence must document `TASKLOOM_MULTI_WRITER_RUNTIME_ENABLEMENT_DECISION`, `TASKLOOM_MULTI_WRITER_RUNTIME_ENABLEMENT_APPROVER`, `TASKLOOM_MULTI_WRITER_RUNTIME_ENABLEMENT_ROLLOUT_WINDOW`, `TASKLOOM_MULTI_WRITER_RUNTIME_ENABLEMENT_MONITORING_SIGNOFF`, `TASKLOOM_MULTI_WRITER_RUNTIME_ENABLEMENT_ABORT_PLAN`, and `TASKLOOM_MULTI_WRITER_RUNTIME_ENABLEMENT_RELEASE_TICKET`.
9. Implement and enable multi-writer/distributed database runtime support only after the Phase 59 approval gate exists and actual runtime support is present, explicitly supported, and approved by release automation.
10. Continue workflow/UI, test, and release hardening.

## Near-Term Definition Of Done

The near-term production hardening track is complete when the Phases 20 through 59 deployment, observability, alerting, workflow-polish, relational-repository, storage-readiness, release-readiness, release-evidence, managed-database-advisory, runtime-guard, handoff-integration, runtime-boundary/foundation, async-boundary-foundation, managed Postgres adapter/backfill foundation, runtime call-site migration evidence, managed Postgres startup support assertion/validation, multi-writer topology requirements/design gate, owned topology design-package gate, design-package review/implementation-authorization evidence gate, implementation-readiness/rollout-safety evidence gate, implementation-scope gate, runtime-implementation validation gate, and release-enable approval gate milestones are paired with actual deployment topology where needed, and the existing route-level RBAC/workflow/job/agent/share parity suite continues to pass on supported storage modes. The relational-repository track now includes Phase 38 mirror retirement for the first six migrated collections, the no-migration jobs scheduler hot-path follow-up, the Phase 39 `providerCalls` cutover and mirror retirement, and the Phase 40 `activationSignals` dedicated-table cutover plus post-Phase-40 mirror retirement. Phase 42 adds the advisory readiness report through `npm run deployment:check-storage`, admin API, and Operations UI, Phase 43 layers `npm run deployment:check-release` and strict mode over backup/restore readiness, persistent paths, and storage topology, Phase 44 packages redacted handoff evidence through `npm run deployment:export-evidence`, Phase 45 adds `npm run deployment:check-managed-db` with strict mode for managed database topology advisory reporting, Phase 46 adds `npm run deployment:check-runtime-guard` with strict mode for blocking unsupported managed database/runtime hints before startup or release automation, Phase 47 integrates Phase 45/46 findings and attachments into the release-readiness and release-evidence handoff story, Phase 48 adds the fail-closed managed database boundary to the synchronous store runtime, Phase 49 lands the async store boundary foundation, Phase 50 lands the managed Postgres document-store adapter/backfill foundation with `db:backfill-managed-postgres` and `db:verify-managed-postgres`, Phase 51 makes runtime call-site migration status visible in the same handoff evidence with no remaining tracked sync call-site groups, Phase 52 asserts/validates managed Postgres startup support for recognized Postgres adapter + managed database URL configurations, Phase 53 captures horizontal writers, regional failover, PITR, and active-active needs as blocked requirements until a real topology is designed and owned, Phase 54 requires owner, consistency model, failover/PITR plan, migration/backfill plan, observability requirements, and rollback strategy before implementation planning, Phase 55 requires reviewed package evidence, approvers, authorization scope, approval timing, and release-evidence attachment before runtime implementation can start, Phase 56 requires implementation readiness, staged rollout controls, rollback/recovery proof, observability proof, and release-claim boundaries before runtime support can be claimed, Phase 57 requires Phase 56 dependency, implementation scope lock, runtime feature flag/deployment gate, validation evidence, migration/cutover lock, and release-owner signoff before implementation claims can proceed, Phase 58 requires Phase 57 complete plus runtime implementation evidence, consistency validation evidence, failover validation evidence, data-integrity validation evidence, operations runbook, and runtime release signoff before runtime-implementation validation can pass, and Phase 59 requires Phase 58 complete plus release-enable decision, approver, rollout window, monitoring signoff, abort plan, and release ticket before release-enable approval can pass. Local JSON/default and supported single-node SQLite remain allowed, narrow managed Postgres startup is accepted only under the Phase 52 assertion rule, and SQLite remains a local single-node posture, not a multi-writer or distributed production topology. Phase 59 still does not enable active-active/regional/PITR runtime support, multi-writer runtime support, or release approval by itself; those remain blocked until actual runtime support is present, explicitly supported, and approved by release automation.
