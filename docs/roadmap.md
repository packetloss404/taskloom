...
- Dedicated relational repository for `activationSignals`: SQLite migration `0017_activation_signals.sql` plus the existing `activationSignalRepository()` API now backed by `activation_signals` in SQLite mode. JSON mode keeps the inline `data.activationSignals` collection, and operator CLIs `db:backfill-activation-signals` and `db:verify-activation-signals` cover cutover recovery and drift audits.
...
- `activationSignals` is part of the JSON app model and is persisted through the opt-in SQLite app runtime. Phase 40 adds a dedicated `activation_signals` table for the SQLite repository path while JSON mode keeps the inline collection.
- `activationSignalRepository()` provides normalized list/upsert access for JSON and SQLite modes; the SQLite implementation now uses `activation_signals` with legacy `app_records` fallback during the cutover window.
...
Phase 40 lifts `activationSignals` from the `app_records` compatibility layer into a dedicated SQLite table while preserving the JSON-default runtime and the existing `activationSignalRepository()` API:
...
- Direct service writes through `upsertActivationSignal(data, ...)` continue to mutate the hydrated `TaskloomData` shape and dual-write to the dedicated table after SQLite `mutateStore()` commits.
- Two new operator CLI commands: `npm run db:backfill-activation-signals [-- --dry-run]` and `npm run db:verify-activation-signals`.

This phase does not retire the JSON-side mirror write for `activationSignals`, change activation fact/read-model storage, normalize signal `data` into separate columns, or make SQLite a multi-writer or distributed production topology.
...
- [x] Start the Phase 40 `activationSignals` relational-repository cutover with a dedicated SQLite table behind the existing repository API, SQLite dual-write from service helpers, and `db:backfill-activation-signals` / `db:verify-activation-signals`.
