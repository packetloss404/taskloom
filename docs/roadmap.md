...
- `docs/deployment-managed-database-topology.md` documents the Phase 45 managed database topology advisory exposed through `npm run deployment:check-managed-db` and `npm run deployment:check-managed-db -- --strict`. The advisory accepts operator hints such as `TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, `TASKLOOM_MANAGED_DATABASE_ADAPTER`, `TASKLOOM_DATABASE_TOPOLOGY`, and `TASKLOOM_STORE`, but it does not add distributed SQLite, Taskloom-owned regional database failover/PITR runtime behavior, active-active multi-region writes, cutover automation, final release closure, runtime activation, or release approval. Phase 62 completes managed Postgres horizontal app-writer concurrency hardening, Phase 63 enforces distributed operational dependencies before strict activation, Phase 64 validates provider-owned HA/PITR recovery behavior, and Phases 65 through 66 remain for cutover automation and final release closure.
- `docs/deployment-managed-database-runtime-guard.md` documents the Phase 46/63/64 managed database runtime guard exposed through `npm run deployment:check-runtime-guard` and `npm run deployment:check-runtime-guard -- --strict`. The guard allows local JSON/default, single-node SQLite, and Phase 52 managed Postgres startup only when a recognized Postgres adapter hint is paired with a managed database URL and the startup support assertion is present, and strict activation remains blocked until the Phase 63 distributed dependency posture is production-safe and Phase 64 recovery validation is complete. Unsupported `TASKLOOM_STORE` values, unpaired managed database URL hints, managed/multi-writer topology hints, local-only dependency posture, and `TASKLOOM_UNSUPPORTED_MANAGED_DB_RUNTIME_BYPASS` remain blocked or break-glass/dev-only rather than production support. Phase 62 completes managed Postgres horizontal app-writer concurrency hardening for the supported posture, while active-active multi-region writes, Taskloom-owned regional database failover/PITR runtime behavior, distributed SQLite, cutover automation, final release closure, runtime activation, and release approval remain blocked until the relevant later phases complete.
...
This means Phase 60 is support-presence assertion evidence only. It records that the implementation and support package are visible for audit, but it does not activate the runtime. Phase 61 adds runtime activation controls and activation-ready reporting on top of this package. Phase 62 completes managed Postgres horizontal app-writer concurrency hardening. Phase 63 enforces distributed dependency posture. Recovery validation, cutover automation, and final release closure remain the bounded Phase 64 through Phase 66 completion track.
...
- Phase 64 is complete for managed Postgres recovery validation. It does not complete cutover/rollback automation, observability automation, final release closure, runtime activation, or release approval. Phase 65 is next.
...
5. Phase 65: automate cutover, rollback, smoke checks, and observability. This is next.
