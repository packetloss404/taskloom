...
- `docs/deployment-release-readiness.md` documents the Phase 43/47/63/64 deployment release-readiness gate exposed through `npm run deployment:check-release` and `npm run deployment:check-release -- --strict`. The gate checks backup/restore readiness, persistent paths, storage topology, Phase 45 managed database advisory findings, Phase 46 runtime-guard findings, Phase 63 distributed dependency posture, and Phase 64 recovery validation posture, but it does not perform backups, restore data, provision managed DBs, add managed database support, or make SQLite distributed.
- `docs/deployment-release-evidence.md` documents the Phase 44/47/63/64 release evidence bundle export exposed through `npm run deployment:export-evidence` and `npm run deployment:export-evidence -- --strict`. The export packages readiness, topology, managed database advisory, runtime-guard, distributed dependency posture, recovery validation posture, and environment evidence for handoff, redacts secrets, and does not run backups, tests, or builds; restore data; provision managed DBs; add managed database support; or make SQLite distributed.
- `docs/deployment-managed-database-topology.md` documents the Phase 45 managed database topology advisory exposed through `npm run deployment:check-managed-db` and `npm run deployment:check-managed-db -- --strict`. The advisory accepts operator hints such as `TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, `TASKLOOM_MANAGED_DATABASE_ADAPTER`, `TASKLOOM_DATABASE_TOPOLOGY`, and `TASKLOOM_STORE`, but it does not add distributed SQLite, Taskloom-owned regional database failover/PITR runtime behavior, or active-active multi-region writes. Phase 62 completes managed Postgres horizontal app-writer concurrency hardening, Phase 63 enforces distributed operational dependencies before strict activation, Phase 64 validates provider-owned HA/PITR recovery behavior, Phase 65 completes cutover/rollback/observability automation, and Phase 66 closes the documentation/release posture for the supported topology.
- `docs/deployment-managed-database-runtime-guard.md` documents the Phase 46/63/64/65 managed database runtime guard exposed through `npm run deployment:check-runtime-guard` and `npm run deployment:check-runtime-guard -- --strict`. The guard allows local JSON/default, single-node SQLite, and Phase 52 managed Postgres startup only when a recognized Postgres adapter hint is paired with a managed database URL and the startup support assertion is present, and strict activation remains blocked unless the Phase 63 distributed dependency posture is production-safe, Phase 64 recovery validation is complete, and Phase 65 cutover preflight/dry-run/smoke/rollback/observability evidence is complete. Unsupported `TASKLOOM_STORE` values, unpaired managed database URL hints, managed/multi-writer topology hints, local-only dependency posture, failed preflight, failed smoke checks, and `TASKLOOM_UNSUPPORTED_MANAGED_DB_RUNTIME_BYPASS` remain blocked or break-glass/dev-only rather than production support. Phase 62 completes managed Postgres horizontal app-writer concurrency hardening for the supported posture; active-active multi-region writes, Taskloom-owned regional database failover/PITR runtime behavior, and distributed SQLite remain outside the supported posture and are future product enhancements.
...
This means Phase 58 is runtime-implementation validation evidence only. It validates the evidence package after Phase 57, but it does not enable active-active writes, regional failover, or PITR runtime support beyond the supported provider-owned managed Postgres posture. Those remain future topology enhancements unless a new roadmap explicitly implements them.
...
This means Phase 60 is support-presence assertion evidence only. It records that the implementation and support package are visible for audit, but it does not activate the runtime by itself. Phase 61 adds runtime activation controls and activation-ready reporting on top of this package. Phase 62 completes managed Postgres horizontal app-writer concurrency hardening. Phase 63 enforces distributed dependency posture. Phase 64 completes managed Postgres recovery validation. Phase 65 completes cutover automation. Phase 66 completes final release closure.
...
- [x] Land Phase 58 multi-writer runtime-implementation validation gate, requiring Phase 57 completion plus runtime implementation evidence, consistency validation evidence, failover validation evidence, data-integrity validation evidence, operations runbook, and runtime release signoff while keeping active-active/regional/PITR runtime support outside the supported posture.
...
- [x] Land Phase 60 multi-writer support-presence assertion gate, requiring Phase 59 completion plus implementation-present assertion, explicit support statement, compatibility matrix, cutover evidence, release automation approval, and owner acceptance evidence while keeping activation dependent on Phases 61 through 66.
- [x] Land Phase 61 runtime activation controls and activation-ready reporting, requiring Phase 60 completion plus `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_DECISION`, `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_OWNER`, `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_WINDOW`, `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_FLAG`, and `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_RELEASE_AUTOMATION_ASSERTION` while keeping concurrency hardening, distributed dependency enforcement, recovery validation, cutover automation, final release closure, runtime support, and release approval blocked at that phase.
...
- [x] Add Phase 58 documentation that captures multi-writer runtime-implementation validation evidence while keeping active-active/regional/PITR runtime support outside the supported posture.
...
- [x] Complete Phase 61 runtime activation controls and activation-ready reporting without completing concurrency hardening, distributed dependency enforcement, recovery validation, cutover automation, final release closure, runtime support, or release approval.
- [x] Complete Phase 62 managed Postgres horizontal app-writer concurrency hardening without enabling active-active multi-region writes, regional failover, PITR runtime support, distributed SQLite, distributed dependency enforcement, recovery validation, cutover automation, final release closure, or release approval.
- [x] Complete Phase 63 distributed dependency enforcement for activation without completing failover/PITR recovery validation, cutover automation, final release closure, runtime activation, or release approval.
- [x] Complete Phase 64 failover, PITR, and recovery validation.
- [x] Complete Phase 65 cutover, rollback, and observability automation.
- [x] Complete Phase 66 final release closure and documentation freeze.
...
Phases 61 through 66 are the bounded completion track for turning the Phase 53 through Phase 60 evidence gates into an explicitly supported production posture. Phase 61 is complete for activation controls and activation-ready reporting only, Phase 62 is complete for managed Postgres horizontal app-writer concurrency hardening, Phase 63 is complete for distributed dependency enforcement before strict activation, Phase 64 is complete for managed Postgres recovery validation with provider-owned HA/PITR evidence, Phase 65 is complete for cutover/rollback/observability automation, and Phase 66 is complete for final release closure/documentation freeze. No additional hidden completion phase is required for the supported production posture.
...
- The concrete inputs are `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_DECISION`, `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_OWNER`, `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_WINDOW`, `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_FLAG`, and `TASKLOOM_MULTI_WRITER_RUNTIME_ACTIVATION_RELEASE_AUTOMATION_ASSERTION`.
...
- Phase 61 exits at activation controls and activation-ready reporting. It does not complete managed Postgres concurrency hardening, distributed dependency enforcement, failover/PITR recovery validation, cutover/rollback automation, observability automation, final release closure, multi-writer runtime support, or release approval.
...
- Phase 62 is complete for the supported managed Postgres horizontal app-writer posture. It does not enable active-active multi-region writes, regional failover, PITR runtime support, distributed SQLite, distributed dependency enforcement, recovery validation, cutover automation, final release closure, or release approval.
...
- The concrete Phase 63 evidence inputs include `TASKLOOM_DISTRIBUTED_RATE_LIMIT_EVIDENCE`, `TASKLOOM_SCHEDULER_COORDINATION_EVIDENCE`, `TASKLOOM_DURABLE_JOB_EXECUTION_POSTURE`, `TASKLOOM_DURABLE_JOB_EXECUTION_EVIDENCE`, `TASKLOOM_ACCESS_LOG_SHIPPING_EVIDENCE`, `TASKLOOM_ALERT_DELIVERY_EVIDENCE`, and `TASKLOOM_HEALTH_MONITORING_EVIDENCE`, alongside the existing distributed limiter, scheduler HTTP leader, access-log, and alert webhook configuration.
...
- Phase 63 is complete for dependency enforcement. Strict activation now depends on distributed rate limiting, HTTP scheduler coordination for horizontally scaled deployments, durable job execution posture, shipped access logs, configured alert delivery, and live health monitoring signals. It does not complete failover/PITR recovery validation, cutover/rollback automation, observability automation beyond the dependency-state surface, final release closure, runtime activation, or release approval.
...
- The concrete Phase 64 evidence inputs are `TASKLOOM_MANAGED_POSTGRES_BACKUP_RESTORE_EVIDENCE`, `TASKLOOM_MANAGED_POSTGRES_PITR_REHEARSAL_EVIDENCE`, `TASKLOOM_MANAGED_POSTGRES_FAILOVER_REHEARSAL_EVIDENCE`, `TASKLOOM_MANAGED_POSTGRES_DATA_INTEGRITY_VALIDATION_EVIDENCE`, and `TASKLOOM_MANAGED_POSTGRES_RECOVERY_TIME_EXPECTATION`.
- Document the supported posture as horizontal Taskloom app writers against one managed Postgres primary/cluster, with HA, failover mechanics, backups, and PITR owned by the database provider and validated by operator rehearsals.
- Keep active-active writes, regional database failover support, distributed SQLite, and any Taskloom-owned PITR runtime behavior blocked unless a later implementation explicitly supports them.
- Exit when recovery claims are explicit, tested, and reflected in release readiness.
- Phase 64 is complete for managed Postgres recovery validation. It does not complete cutover/rollback automation, observability automation, final release closure, runtime activation, or release approval.
...
- The canonical Phase 65 evidence inputs are `TASKLOOM_CUTOVER_PREFLIGHT_STATUS` with `TASKLOOM_CUTOVER_PREFLIGHT_EVIDENCE`, `TASKLOOM_ACTIVATION_DRY_RUN_STATUS` with `TASKLOOM_ACTIVATION_DRY_RUN_EVIDENCE`, `TASKLOOM_POST_ACTIVATION_SMOKE_STATUS` with `TASKLOOM_POST_ACTIVATION_SMOKE_EVIDENCE`, `TASKLOOM_ROLLBACK_COMMAND_GUIDANCE`, `TASKLOOM_MONITORING_THRESHOLDS`, and `TASKLOOM_SMOKE_FAILURE_ROLLBACK_EVIDENCE` when failed automation must prove rollback to the prior safe posture.
- Phase 65 is complete for the supported managed Postgres cutover flow: preflight failures keep activation blocked, smoke-check failures trigger rollback to the prior safe posture, and operations health/release evidence expose the activation/cutover status. It does not enable active-active multi-region writes, Taskloom-owned regional database failover/PITR runtime behavior, or distributed SQLite. Phase 66 closes the supported release posture after this automation.

### Phase 66: Final Release Closure

- Phase 66 is complete for final release closure and documentation freeze.
- The docs now describe the supported production posture as complete without implying another required phase.
- Any topology outside the supported posture is a future product enhancement, not a hidden release blocker for this posture.

## Final Release Checklist, Topology, And Boundaries

Phase 66 freezes the supported production posture in one place:

- Supported topology: local JSON/default for contributor and local workflows; supported single-node SQLite with durable local disk, WAL/backup/restore discipline, and no distributed database claims; and managed Postgres startup for recognized Postgres adapter + managed database URL configurations.
- Supported managed Postgres production posture: multiple Taskloom app processes may write horizontally to one managed Postgres primary/cluster when activation controls, production-safe shared dependencies, provider-owned HA/PITR recovery evidence, cutover preflight, activation dry-run, post-activation smoke checks, rollback guidance, and monitoring thresholds are complete.
- Required final checklist: run typecheck, tests, build, deployment CLI checks (`deployment:check-storage`, `deployment:check-managed-db`, `deployment:check-runtime-guard`, `deployment:check-release`, and `deployment:export-evidence`, using `-- --strict` where release automation requires fail-closed behavior), and docs consistency searches before release handoff.
- Runtime boundaries: SQLite remains single-node; scheduler coordination and distributed rate limiting coordinate their own scopes but do not make SQLite distributed; managed/Postgres hints require a recognized Postgres adapter plus managed database URL; unsupported store names, unpaired database URL hints, local-only dependency posture under strict activation, failed preflight, failed smoke checks, and production use of `TASKLOOM_UNSUPPORTED_MANAGED_DB_RUNTIME_BYPASS` remain blocked.
- Unsupported/future topology: active-active multi-region database writes, Taskloom-owned regional database failover, Taskloom-owned PITR runtime behavior, distributed SQLite, and any topology broader than horizontal Taskloom app writers against one managed Postgres primary/cluster are future product enhancements unless a new roadmap explicitly chooses, implements, validates, and documents them.
- Release boundary: the completion track is closed for the supported posture above. Remaining roadmap items are product, maintenance, or future topology enhancements rather than required hidden phases for production release closure.

## Recommended Order

1. Phase 61: add runtime activation controls on top of completed Phase 60 evidence. Complete for activation-ready reporting only.
2. Phase 62: harden managed Postgres horizontal-writer concurrency. Complete for managed Postgres horizontal app writers only.
3. Phase 63: enforce distributed operational dependencies before activation. Complete for strict activation dependency posture only.
4. Phase 64: validate failover, PITR, backup restore, and recovery behavior. Complete for provider-owned managed Postgres HA/PITR only.
5. Phase 65: automate cutover, rollback, smoke checks, and observability. Complete for repeatable activate, verify, and rollback flow only.
6. Phase 66: close the release and freeze the docs so there is no implied follow-up phase for the supported production posture. Complete.
