...
- `docs/deployment-managed-database-topology.md` documents the Phase 45 managed database topology advisory exposed through `npm run deployment:check-managed-db` and `npm run deployment:check-managed-db -- --strict`. The advisory accepts operator hints such as `TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, `TASKLOOM_MANAGED_DATABASE_ADAPTER`, `TASKLOOM_DATABASE_TOPOLOGY`, and `TASKLOOM_STORE`, but it does not add distributed SQLite, Taskloom-owned regional database failover/PITR runtime behavior, active-active multi-region writes, final release closure, runtime activation, or release approval. Phase 62 completes managed Postgres horizontal app-writer concurrency hardening, Phase 63 enforces distributed operational dependencies before strict activation, Phase 64 validates provider-owned HA/PITR recovery behavior, Phase 65 completes cutover/rollback/observability automation, and Phase 66 remains for final release closure.
- `docs/deployment-managed-database-runtime-guard.md` documents the Phase 46/63/64/65 managed database runtime guard exposed through `npm run deployment:check-runtime-guard` and `npm run deployment:check-runtime-guard -- --strict`. The guard allows local JSON/default, single-node SQLite, and Phase 52 managed Postgres startup only when a recognized Postgres adapter hint is paired with a managed database URL and the startup support assertion is present, and strict activation remains blocked until the Phase 63 distributed dependency posture is production-safe, Phase 64 recovery validation is complete, and Phase 65 cutover preflight/dry-run/smoke/rollback/observability evidence is complete. Unsupported `TASKLOOM_STORE` values, unpaired managed database URL hints, managed/multi-writer topology hints, local-only dependency posture, failed preflight, failed smoke checks, and `TASKLOOM_UNSUPPORTED_MANAGED_DB_RUNTIME_BYPASS` remain blocked or break-glass/dev-only rather than production support. Phase 62 completes managed Postgres horizontal app-writer concurrency hardening for the supported posture, while active-active multi-region writes, Taskloom-owned regional database failover/PITR runtime behavior, distributed SQLite, final release closure, runtime activation, and release approval remain blocked until the relevant later phase completes.
...
- [x] Land Phase 60 multi-writer support-presence assertion gate, requiring Phase 59 completion plus implementation-present assertion, explicit support statement, compatibility matrix, cutover evidence, release automation approval, and owner acceptance evidence while keeping runtime support blocked until later phases complete.
...
- Phase 65 is complete for the supported managed Postgres cutover flow: preflight failures keep activation blocked, smoke-check failures trigger rollback to the prior safe posture, and operations health/release evidence expose the activation/cutover status. It does not enable active-active multi-region writes, Taskloom-owned regional database failover/PITR runtime behavior, distributed SQLite, final release closure, runtime activation, or release approval. Phase 66 is next.
...
The final completion track is complete when Phases 64 through 66 have landed after completed Phases 61, 62, and 63. At that point the supported production posture must have safe managed Postgres horizontal-writer behavior, enforced distributed dependencies, validated backup/restore/PITR/failover recovery behavior, repeatable cutover and rollback automation, operations visibility, full validation, and documentation that clearly says the project is complete for that supported posture.
