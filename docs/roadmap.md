...
- Managed Postgres, regional failover, PITR, active/active writes, and any multi-writer database runtime remain future implementation work.
...
- The guard allows only the current local JSON/default runtime and single-node SQLite runtime.
...
- The integration keeps the support boundary explicit: local JSON/default storage and supported single-node SQLite remain allowed, while managed database requests, managed database URL hints, and multi-writer topology hints are blocked or advisory until managed database runtime support is implemented.
...
- Local JSON/default storage and supported single-node SQLite remain the only implemented runtime postures.
- Managed/Postgres store modes and managed database URL hints remain blocked or advisory for the main app sync startup path until managed Postgres startup support is explicitly asserted and covered.
...
This means Phase 50 can land the managed Postgres adapter/backfill foundation now that the async boundary foundation is in place. Managed database hints still need to remain blocked in the main app startup path until the guarded synchronous runtime call sites move onto the managed database path.

### Phase 50 Managed Postgres Adapter/Backfill Foundation

Phase 50 lands the managed Postgres adapter/backfill foundation after the Phase 49 async store boundary. It is the first managed-database implementation foundation, not the point where managed/Postgres hints become supported app startup configuration:

- The milestone is documented as landed for the managed Postgres adapter/backfill foundation.
- The current main app startup path still runs the Phase 46 runtime guard before serving traffic, so `TASKLOOM_STORE=managed`, `TASKLOOM_STORE=managed-db`, `TASKLOOM_STORE=managed-database`, `TASKLOOM_STORE=postgres`, `TASKLOOM_STORE=postgresql`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, and `TASKLOOM_MANAGED_DATABASE_URL` remain blocked as normal startup inputs in this branch.
...
- Phase 51 migrates tracked runtime call sites to the async/managed database path. The guard can honestly allow managed/Postgres startup only after startup support is explicitly asserted and covered.
...
- [x] Land Phase 50 managed Postgres adapter/backfill foundation with the `pg` dependency, `TASKLOOM_MANAGED_DATABASE_ADAPTER`, `db:backfill-managed-postgres`, and `db:verify-managed-postgres`. The main app sync startup path remains guarded until managed/Postgres startup support is explicitly asserted and covered.
...
- [x] Add Phase 51 deployment/reporting evidence for runtime call-site migration, while preserving strict managed/Postgres startup blockers because startup support is not yet explicitly asserted.
- [ ] Continue production hardening implementation beyond process/store-scoped rate limiting, webhook email delivery, CSRF checks, app-level redaction, Phase 20 export/access-log controls, Phase 21 scheduler leader election, Phase 22 managed external email provider operations, Phase 23 access-log shipping/retention, Phase 24 operational status/health endpoints, Phase 42 storage topology readiness reporting, Phase 43 release-readiness gates, Phase 44 release evidence exports, Phase 45 managed database topology advisory reporting, Phase 46 runtime guardrails, Phase 47 handoff integration, Phase 48 runtime boundary/foundation, Phase 49 async store boundary foundation, Phase 50 managed Postgres adapter/backfill foundation, and Phase 51 runtime call-site migration evidence. The remaining storage gaps are explicit managed Postgres startup support/validation and multi-writer topology support where horizontal writers, regional failover, PITR, or active/active writes are required. SQLite/database topology guidance lives in `docs/deployment-sqlite-topology.md`, the Phase 42 advisory workflow lives in `docs/deployment-storage-topology.md`, the Phase 43/47 release gate lives in `docs/deployment-release-readiness.md`, the Phase 44/47 evidence bundle lives in `docs/deployment-release-evidence.md`, the Phase 45 managed database advisory lives in `docs/deployment-managed-database-topology.md`, and the Phase 46/48/49/50/51 runtime boundary, guard, and call-site migration story lives in `docs/deployment-managed-database-runtime-guard.md`.
...
1. Assert and validate managed/Postgres startup support before relaxing the startup guard.
2. Design the multi-writer topology only if production requirements need horizontal database writers, regional failover, PITR, or active/active writes; scheduler leader election and distributed rate limiting do not solve database write coordination by themselves.
3. Continue workflow/UI, test, and release hardening.

## Near-Term Definition Of Done

The near-term production hardening track is complete when the Phases 20 through 51 deployment, observability, alerting, workflow-polish, relational-repository, storage-readiness, release-readiness, release-evidence, managed-database-advisory, runtime-guard, handoff-integration, runtime-boundary/foundation, async-boundary-foundation, managed Postgres adapter/backfill foundation, and runtime call-site migration evidence milestones are paired with actual managed database topology where needed, and the existing route-level RBAC/workflow/job/agent/share parity suite continues to pass on supported storage modes. The relational-repository track now includes Phase 38 mirror retirement for the first six migrated collections, the no-migration jobs scheduler hot-path follow-up, the Phase 39 `providerCalls` cutover and mirror retirement, and the Phase 40 `activationSignals` dedicated-table cutover plus post-Phase-40 mirror retirement. Phase 42 adds the advisory readiness report through `npm run deployment:check-storage`, admin API, and Operations UI, Phase 43 layers `npm run deployment:check-release` and strict mode over backup/restore readiness, persistent paths, and storage topology, Phase 44 packages redacted handoff evidence through `npm run deployment:export-evidence`, Phase 45 adds `npm run deployment:check-managed-db` with strict mode for managed database topology advisory reporting, Phase 46 adds `npm run deployment:check-runtime-guard` with strict mode for blocking unsupported managed database/runtime hints before startup or release automation, Phase 47 integrates Phase 45/46 findings and attachments into the release-readiness and release-evidence handoff story, Phase 48 adds the fail-closed managed database boundary to the synchronous store runtime, Phase 49 lands the async store boundary foundation, Phase 50 lands the managed Postgres document-store adapter/backfill foundation with `db:backfill-managed-postgres` and `db:verify-managed-postgres`, and Phase 51 makes runtime call-site migration status visible in the same handoff evidence with no remaining tracked sync call-site groups. The remaining startup gap is explicit managed Postgres startup support: local JSON/default and supported single-node SQLite remain allowed, while managed database requests are still blocked/advised by the main app startup guard until startup support is explicitly asserted and covered. SQLite remains a local single-node posture, not a multi-writer or distributed production topology.
