...
- [ ] Continue production hardening implementation beyond process/store-scoped rate limiting, webhook email delivery, CSRF checks, app-level redaction, Phase 20 export/access-log controls, Phase 21 scheduler leader election, and Phase 22 managed external email provider operations, such as managed production topology support and dedicated relational repositories where indexed `app_records` metadata is not enough. SQLite/database topology guidance lives in `docs/deployment-sqlite-topology.md`.
...
2. Managed log shipping, retention, and SIEM integration around the Phase 20 access-log middleware, workspace export pipeline, and proxy access-log redaction templates.
3. Continued workflow/UI, test, and release hardening.

## Near-Term Definition Of Done

The near-term production hardening track is complete when app-level token-redaction enforcement, the Phase 20 export/access-log redaction controls (in-app middleware, workspace export pipeline, proxy templates, and validator), the Phase 21 scheduler leader-election gate (`off`/`file`/`http` modes with documented wire protocol and validation checklist), and the Phase 22 managed external email provider operations (inbound provider-status webhook gated by `TASKLOOM_INVITATION_EMAIL_RECONCILIATION_SECRET` plus the `jobs:reconcile-invitation-emails` CLI) are paired with deployment-owned managed log shipping/retention, managed database topology is implemented where needed, dedicated relational repositories/backfills are added where indexed `app_records` metadata is not enough, and the existing route-level RBAC/workflow/job/agent/share parity suite continues to pass on both storage modes. The remaining gaps are managed database topology, dedicated relational repositories, managed log shipping, and continued workflow/UI polish.
