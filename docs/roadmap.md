...
- Phase 50 lands the managed Postgres document-store adapter/backfill foundation with `pg`, `TASKLOOM_MANAGED_DATABASE_ADAPTER`, `db:backfill-managed-postgres`, and `db:verify-managed-postgres`. The main app sync startup path is still guarded by the managed database runtime guard, so Phase 51+ must migrate runtime call sites before managed/Postgres hints become supported app startup configuration.
...
- Local JSON/default storage and supported single-node SQLite remain the only implemented runtime postures.
- Managed/Postgres store modes and managed database URL hints remain blocked or advisory for the main app sync startup path until Phase 51+ migrates runtime call sites.
...
- Phase 51+ should migrate runtime call sites to the async/managed database path before the guard can honestly allow managed/Postgres startup.
...
- [x] Land Phase 50 managed Postgres adapter/backfill foundation with the `pg` dependency, `TASKLOOM_MANAGED_DATABASE_ADAPTER`, `db:backfill-managed-postgres`, and `db:verify-managed-postgres`. The main app sync startup path remains guarded, and Phase 51+ must migrate runtime call sites before managed/Postgres hints become supported app configuration.
...
1. Phase 51+: migrate runtime call sites onto the async/managed database path, then relax the startup guard only when managed/Postgres startup is executable and covered.
