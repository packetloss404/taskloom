...
- Phase 48 adds the managed database runtime boundary/foundation inside the synchronous app-store backend selection: `TASKLOOM_STORE=managed`, `TASKLOOM_STORE=postgres`, and managed database URL hints are recognized as managed intent and rejected with the sync-adapter gap until the store API is migrated to an async managed database adapter.
...
This means Phase 47 completed the release-readiness/evidence handoff integration for managed-database advisory and guard findings, and Phase 48 completed the runtime boundary/foundation that prevents the synchronous app-store path from silently accepting managed database hints. The remaining production storage gaps are the async store migration, the actual managed Postgres adapter/repository/backfill work, and the multi-writer topology story if Taskloom needs horizontal writers or regional failover.
