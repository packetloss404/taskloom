...
- `docs/deployment-managed-database-topology.md` documents the Phase 45 managed database topology advisory exposed through `npm run deployment:check-managed-db` and `npm run deployment:check-managed-db -- --strict`. The advisory accepts operator hints such as `TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, `TASKLOOM_DATABASE_TOPOLOGY`, and `TASKLOOM_STORE`, but it does not add a managed Postgres adapter, distributed SQLite, multi-writer DB runtime, regional failover, PITR, or active/active writes.
...
- Phase 49 lands the async store boundary foundation after that fail-closed managed-database boundary. It prepares the codebase for Phase 50 managed Postgres adapter/repository/backfill work, but it does not add managed Postgres runtime support.
...
- The async managed-database backend still rejects managed/Postgres store modes and managed database URL hints with `ManagedDatabaseStoreBoundaryError` until a real adapter exists.
- Local JSON/default storage and supported single-node SQLite remain the only implemented runtime postures.
- Managed/Postgres store modes and managed database URL hints remain blocked or advisory until a real managed database adapter exists.
...
This means Phase 50 should be the actual managed Postgres adapter/repositories/backfills phase now that the async boundary foundation is in place. Phase 50 still needs implementation, parity tests, migration/backfill/verify commands, backup/restore guidance, release evidence, and guard/readiness updates before managed database hints can become supported runtime configuration.
...
- [x] Land Phase 49 async store boundary foundation after the fail-closed managed database boundary, so Phase 50 can focus on the actual managed Postgres adapter/repositories/backfills. This does not add managed Postgres runtime support, managed database repositories/backfills, distributed SQLite, regional failover, PITR, active/active writes, or multi-writer database support.
...
1. Phase 50: implement the actual managed Postgres adapter plus managed-database repositories, migrations/backfills, parity tests, backup/restore guidance, release evidence, and guard/readiness updates now that the async boundary foundation exists.
