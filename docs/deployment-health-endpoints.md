...
- `managedPostgresRecoveryValidation` summarizes Phase 64. `recoveryValidationComplete: true` means backup restore, PITR rehearsal, provider failover rehearsal, data-integrity validation, and recovery-time expectations are present after Phase 63 dependencies are production-safe. Canonical envs are `TASKLOOM_MANAGED_POSTGRES_BACKUP_RESTORE_EVIDENCE`, `TASKLOOM_MANAGED_POSTGRES_PITR_REHEARSAL_EVIDENCE`, `TASKLOOM_MANAGED_POSTGRES_FAILOVER_REHEARSAL_EVIDENCE`, `TASKLOOM_MANAGED_POSTGRES_DATA_INTEGRITY_VALIDATION_EVIDENCE`, and `TASKLOOM_MANAGED_POSTGRES_RECOVERY_TIME_EXPECTATION`; operations also recognizes `TASKLOOM_RECOVERY_*` aliases for report ingestion.
- `managedPostgresCutoverAutomation` summarizes Phase 65. `cutoverRollbackAutomationReady: true` means the cutover preflight, activation dry-run, post-activation smoke checks, rollback command guidance, monitoring thresholds, operations health cutover status evidence, and prior safe posture proof are present after Phase 64 recovery validation. Failed preflight keeps activation blocked; failed smoke checks require rollback evidence before another attempt.
- Phase 66 finalizes release documentation for the supported managed Postgres horizontal app-writer posture. That posture still does not include active-active multi-region writes, Taskloom-owned regional database failover/PITR runtime behavior, or distributed SQLite.
...
`GET /api/app/operations/health` is admin-scoped and returns per-subsystem health classifications with diagnostic detail. It complements the binary public readiness probe by giving operators actionable per-subsystem status without log-scraping. Phase 63 uses this surface to report `distributedDependencyEnforcement` posture so operators can see whether strict activation is blocked by local-only rate limiting, scheduler coordination, job execution, log shipping, alert delivery, or monitoring. Phase 64 adds `managedPostgresRecoveryValidation` so the same endpoint explains missing backup restore, PITR, failover, data-integrity, or recovery-time evidence. Phase 65 adds `managedPostgresCutoverAutomation` so operators can see failed preflight, failed smoke checks, rollback posture, and complete cutover evidence before activation.
...
The Operations page now displays the operator-status payload as an admin-gated "Production Status" tile. Admins and owners see the rendered tile with store mode, managed database/runtime boundary posture, Phase 49 async store boundary foundation, Phase 50 managed Postgres capability, scheduler leader configuration, jobs queue summary, access-log mode, Phase 63 distributed dependency enforcement posture, Phase 64 managed Postgres recovery-validation posture, and Node version; non-admin members do not see the tile, matching the route-level RBAC on the underlying endpoint. Phase 50's adapter/backfill foundation should not be read as a green light to bypass that boundary tile or the startup guard. The tile is the recommended way for operators to spot store/scheduler/access-log/dependency misconfiguration without SSHing into the host. The frontend wiring lives in `web/src/pages/Operations.tsx`.
