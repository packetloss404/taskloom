...
- async store boundary foundation for the next managed-database implementation step, without adding managed Postgres runtime support
...
- Run `npm run deployment:check-managed-db` before managed production database planning handoff; use `npm run deployment:check-managed-db -- --strict` in automation that should fail on blocking topology findings. Operators can set advisory hints such as `TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, `TASKLOOM_DATABASE_TOPOLOGY`, and `TASKLOOM_STORE` so the report can classify intent. Phase 45 is reporting/planning only: it does not add a managed Postgres adapter, distributed SQLite, multi-writer runtime support, regional failover, PITR, or active/active writes.
- Run `npm run deployment:check-runtime-guard` before startup/release automation; use `npm run deployment:check-runtime-guard -- --strict` in gates that must fail when unsupported runtime hints are present. The Phase 46 guard allows only the current local JSON/default runtime and single-node SQLite runtime. It blocks unsupported `TASKLOOM_STORE` values, managed database URL hints (`TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`), and managed/multi-writer topology hints (`TASKLOOM_DATABASE_TOPOLOGY`) before they can be mistaken for supported runtime configuration. `TASKLOOM_UNSUPPORTED_MANAGED_DB_RUNTIME_BYPASS` is a break-glass/dev-only escape hatch for controlled experiments; it is not production support and must not be used to claim managed Postgres, distributed SQLite, or multi-writer support. Phase 48 also adds a runtime boundary inside the synchronous app-store path: managed/Postgres store modes or managed database URL hints are rejected by the current `loadStore()`/`mutateStore()` backend selection. Phase 49 lands the async store boundary foundation that Phase 50 can build on, but it still does not add managed Postgres runtime support.
