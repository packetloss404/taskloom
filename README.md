...
- managed database runtime boundary/foundation in the synchronous store path so managed/Postgres hints fail closed until a real adapter exists
- async store boundary foundation plus Phase 50 managed Postgres adapter/backfill foundation; Phase 51 completes the tracked runtime call-site migration while the main app startup path still guards managed/Postgres hints until startup support is explicitly asserted and covered
...
Phase 50 documents the managed Postgres adapter/backfill foundation, and Phase 51 completes the tracked runtime call-site migration needed after that foundation. `npm start` still runs the managed database runtime guard before the Hono server starts. The landed foundation adds the `pg` adapter dependency, the `TASKLOOM_MANAGED_DATABASE_ADAPTER` advisory/guard hint, and the `db:backfill-managed-postgres` / `db:verify-managed-postgres` CLIs. Do not use `TASKLOOM_STORE=managed`, `TASKLOOM_STORE=managed-db`, `TASKLOOM_STORE=managed-database`, `TASKLOOM_STORE=postgres`, `TASKLOOM_STORE=postgresql`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, or `TASKLOOM_MANAGED_DATABASE_URL` as supported app startup configuration yet. Phase 51 call-site migration is represented in deployment readiness/evidence with no remaining tracked sync call-site groups, but managed/Postgres hints become normal startup configuration only after managed Postgres startup support is explicitly asserted and covered.
...
- Run `npm run deployment:check-managed-db` before managed production database planning handoff; use `npm run deployment:check-managed-db -- --strict` in automation that should fail on blocking topology findings. Operators can set advisory hints such as `TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, `TASKLOOM_MANAGED_DATABASE_ADAPTER`, `TASKLOOM_DATABASE_TOPOLOGY`, and `TASKLOOM_STORE` so the report can classify intent. Phase 45 is reporting/planning only; Phase 50 adds a managed Postgres adapter/backfill foundation, but not distributed SQLite, multi-writer runtime support, regional failover, PITR, or active/active writes.
- Run `npm run deployment:check-runtime-guard` before startup/release automation; use `npm run deployment:check-runtime-guard -- --strict` in gates that must fail when unsupported runtime hints are present. The Phase 46 guard allows only the current local JSON/default runtime and single-node SQLite runtime. It blocks unsupported `TASKLOOM_STORE` values, managed database URL hints (`TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`), and managed/multi-writer topology hints (`TASKLOOM_DATABASE_TOPOLOGY`) before they can be mistaken for supported runtime configuration. It also reports Phase 50 adapter/backfill evidence from `TASKLOOM_MANAGED_DATABASE_ADAPTER` when paired with a managed database URL hint, and Phase 51 call-site migration evidence so release gates can distinguish adapter availability from startup readiness. `TASKLOOM_UNSUPPORTED_MANAGED_DB_RUNTIME_BYPASS` is a break-glass/dev-only escape hatch for controlled experiments; it is not production support and must not be used to claim managed Postgres, distributed SQLite, or multi-writer support. Phase 48 also adds a runtime boundary inside the synchronous app-store path: managed/Postgres store modes or managed database URL hints are rejected by the current `loadStore()`/`mutateStore()` backend selection. Phase 49 lands the async store boundary foundation, Phase 50 lands the managed Postgres adapter/backfill foundation, and Phase 51 reports no remaining tracked sync runtime call-site groups. The main app sync startup path remains guarded in this branch, so managed/Postgres hints are not supported app startup configuration yet.
