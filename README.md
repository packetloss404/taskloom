...
- async store boundary foundation plus Phase 50 managed Postgres adapter/backfill foundation, while the main app startup path still guards managed/Postgres hints until Phase 51+ migrates runtime call sites
...
Phase 50 documents the managed Postgres adapter/backfill foundation, but `npm start` still runs the managed database runtime guard before the Hono server starts. The landed foundation adds the `pg` adapter dependency, the `TASKLOOM_MANAGED_DATABASE_ADAPTER` advisory/guard hint, and the `db:backfill-managed-postgres` / `db:verify-managed-postgres` CLIs. Do not use `TASKLOOM_STORE=managed`, `TASKLOOM_STORE=managed-db`, `TASKLOOM_STORE=managed-database`, `TASKLOOM_STORE=postgres`, `TASKLOOM_STORE=postgresql`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`, or `TASKLOOM_MANAGED_DATABASE_URL` as supported app startup configuration yet. Phase 51+ is expected to migrate runtime call sites onto the async/managed database path before those hints become normal startup configuration.
...
- Run `npm run deployment:check-runtime-guard` before startup/release automation; use `npm run deployment:check-runtime-guard -- --strict` in gates that must fail when unsupported runtime hints are present. The Phase 46 guard allows only the current local JSON/default runtime and single-node SQLite runtime. It blocks unsupported `TASKLOOM_STORE` values, managed database URL hints (`TASKLOOM_MANAGED_DATABASE_URL`, `DATABASE_URL`, `TASKLOOM_DATABASE_URL`), and managed/multi-writer topology hints (`TASKLOOM_DATABASE_TOPOLOGY`) before they can be mistaken for supported runtime configuration. It also reports Phase 50 adapter/backfill evidence from `TASKLOOM_MANAGED_DATABASE_ADAPTER` when paired with a managed database URL hint. `TASKLOOM_UNSUPPORTED_MANAGED_DB_RUNTIME_BYPASS` is a break-glass/dev-only escape hatch for controlled experiments; it is not production support and must not be used to claim managed Postgres, distributed SQLite, or multi-writer support. Phase 48 also adds a runtime boundary inside the synchronous app-store path: managed/Postgres store modes or managed database URL hints are rejected by the current `loadStore()`/`mutateStore()` backend selection. Phase 49 lands the async store boundary foundation, and Phase 50 lands the managed Postgres adapter/backfill foundation. The main app sync startup path remains guarded in this branch, so Phase 51+ still needs to migrate runtime call sites before managed/Postgres hints are supported app startup configuration.
