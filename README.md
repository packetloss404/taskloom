...
Deployment guidance lives in `README.md#production-deployment-guidance`, `docs/deployment-auth-hardening.md`, `docs/deployment-sqlite-topology.md`, `docs/invitation-email-operations.md`, `docs/deployment-export-redaction.md`, `docs/deployment-scheduler-coordination.md`, `docs/deployment-access-log-shipping.md`, `docs/deployment-health-endpoints.md`, and `docs/roadmap-relational-repositories.md`, and is tracked across Phases 16 through 38 in `docs/roadmap.md`. Phase 32 begins the dedicated-relational-repositories migration by moving `jobMetricSnapshots` into a dedicated SQLite table behind a JSON/SQLite-switching repository factory, with dual-write to `app_records` during the cutover window and operator CLIs `db:backfill-job-metric-snapshots` and `db:verify-job-metric-snapshots`. Phase 33 extends the same pattern to `alertEvents` with migration `0011_alert_events.sql`, the `createAlertEventsRepository` factory, dual-write of `recordAlerts` and `updateAlertDeliveryStatus` in SQLite mode, and operator CLIs `db:backfill-alert-events` and `db:verify-alert-events`. Phase 34 extends the same pattern to `agentRuns` with migration `0012_agent_runs.sql`, the `createAgentRunsRepository` factory, dual-write of `upsertAgentRun` in SQLite mode, and operator CLIs `db:backfill-agent-runs` (with optional `--check-orphans`) and `db:verify-agent-runs`. Phase 35 extends the same pattern to `jobs` with migration `0013_jobs.sql` and the `createJobsRepository` factory under a deliberately conservative Option-2 cutover: the dedicated table, dual-write of `enqueueJob`/`updateJob`/`cancelJob`/`enqueueRecurringJob`/`maintainScheduledAgentJobs` plus the state-change emissions from `claimNextJob`/`sweepStaleRunningJobs`, and operator CLIs `db:backfill-jobs` and `db:verify-jobs` all ship now, while the scheduler keeps its existing load-store-loop pattern with `claimMutex` and the repository's `claimNext`/`sweepStaleRunning` SQL-native primitives wait for a future hot-path cutover. Phase 36 extends the same pattern to `invitationEmailDeliveries` with migration `0014_invitation_email_deliveries_table.sql`, the `createInvitationEmailDeliveriesRepository` factory, dual-write of all five mutators (`createInvitationEmailDelivery`, `markInvitationEmailDeliverySent/Skipped/Failed`, `recordInvitationEmailProviderStatus`) via the deferred-queue mechanism inside `mutateSqliteStore`, and operator CLIs `db:backfill-invitation-email-deliveries` and `db:verify-invitation-email-deliveries`. The Phase 22 schema-additive trick stops working past this step — any future field on `InvitationEmailDeliveryRecord` requires an explicit `ALTER TABLE invitation_email_deliveries ADD COLUMN ...` migration. Phase 37 extends the same pattern to `activities` with migration `0015_activities.sql`, `src/repositories/activities-repo.ts`, read delegation preserving `listActivitiesForWorkspaceIndexed(workspaceId, limit?)`, SQLite dual-write to both `app_records` and the dedicated table, and operator CLIs `db:backfill-activities` and `db:verify-activities`. Phase 38 retires the SQLite `app_records` mirrors for `jobMetricSnapshots`, `alertEvents`, `agentRuns`, `jobs`, `invitationEmailDeliveries`, and `activities`; `loadStore()` remains fully hydrated by reading those collections from their dedicated tables, and the existing backfill/verify commands remain available for old-backup recovery.
