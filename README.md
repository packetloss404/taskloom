...
- Phase 53 multi-writer topology requirements/design gate, Phase 54 owned design-package gate, Phase 55 design-package review/implementation-authorization evidence gate, Phase 56 implementation-readiness/rollout-safety evidence gate, Phase 57 implementation-scope gate, Phase 58 runtime-implementation validation gate, Phase 59 release-enable approval gate, Phase 60 support-presence assertion gate, Phase 61 runtime activation controls gate, Phase 62 managed Postgres horizontal app-writer concurrency hardening, Phase 63 distributed dependency enforcement, Phase 64 recovery validation, Phase 65 cutover/rollback/observability automation, and Phase 66 final release closure/documentation freeze for the supported production posture, while active-active multi-region writes, Taskloom-owned regional database failover/PITR runtime behavior, and distributed SQLite remain unsupported topology boundaries
...
- Product Builder Sprint Plan for the next track: prompt-to-agent, prompt-to-app app-draft contracts, live preview, iteration chat, one-click self-hosted publish, integrations marketplace lite, and builder beta polish with first-run checklist, template gallery, model presets, publish dashboard, and timeline affordances. See `docs/product-builder-sprint-plan.md`.
...
- Run `npm run deployment:check-release -- --strict` and `npm run deployment:export-evidence -- --strict` so release handoff evidence includes storage topology, managed database advisory, runtime-guard findings, Phase 63 distributed dependency posture, Phase 64 recovery validation posture, and Phase 65 activation/cutover status.
...
- Prepare Phase 65 cutover artifacts using the canonical evidence names `TASKLOOM_CUTOVER_PREFLIGHT_STATUS` with `TASKLOOM_CUTOVER_PREFLIGHT_EVIDENCE`, `TASKLOOM_ACTIVATION_DRY_RUN_STATUS` with `TASKLOOM_ACTIVATION_DRY_RUN_EVIDENCE`, `TASKLOOM_POST_ACTIVATION_SMOKE_STATUS` with `TASKLOOM_POST_ACTIVATION_SMOKE_EVIDENCE`, `TASKLOOM_ROLLBACK_COMMAND_GUIDANCE`, `TASKLOOM_MONITORING_THRESHOLDS`, and `TASKLOOM_SMOKE_FAILURE_ROLLBACK_EVIDENCE` when failed automation must prove rollback to the prior safe posture.
...
- `docs/roadmap.md`
- `docs/product-builder-sprint-plan.md`
