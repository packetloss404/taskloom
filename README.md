...
- Run `npm run deployment:check-release -- --strict` and `npm run deployment:export-evidence -- --strict` so release handoff evidence includes storage topology, managed database advisory, runtime-guard findings, and Phase 63 distributed dependency posture.
