...
const VALID_PROVIDERS: ApiKeyProvider[] = ["anthropic", "openai", "openrouter", "minimax", "ollama"];
...
apiKeyRoutes.post("/", async (c) => {
  try {
    ...
    if (!body.provider || !VALID_PROVIDERS.includes(body.provider as ApiKeyProvider)) {
      throw httpError(400, "provider must be one of anthropic, openai, openrouter, minimax, ollama");
    }
