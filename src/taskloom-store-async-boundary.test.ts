...
test("async store env helper clears and restores Phase 58 runtime evidence keys", async () => {
  const previous = new Map<StoreEnvKey, string | undefined>();
...
test("async store env helper clears and restores Phase 60 runtime support evidence keys", async () => {
  const previous = new Map<StoreEnvKey, string | undefined>();
...
test("async store env helper clears and restores Phase 61 runtime activation control keys", async () => {
  const previous = new Map<StoreEnvKey, string | undefined>();
...
test("synchronous APIs stay guarded when multi-writer Phase 58 runtime implementation evidence is configured", async () => {
  const blockedTopologies = ["multi-writer", "distributed", "active-active"] as const;
...
test("synchronous APIs stay guarded when complete multi-writer Phase 60 runtime support evidence is configured", async () => {
  const blockedTopologies = ["multi-writer", "distributed", "active-active"] as const;
...
test("synchronous APIs stay guarded when complete multi-writer Phase 61 activation controls are configured", async () => {
