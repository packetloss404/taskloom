...
test("runAgent validates required inputs and records logs", () => {
  ...
  const { run } = runAgent(auth.context, agent.id, { release_label: "v1.2.3", evidence_url: "https://example.com/evidence" });
...
test("runAgent rejects invalid url and enum inputs", () => {
  ...
  assert.throws(
    () => runAgent(auth.context, agent.id, { release_label: "v1", evidence_url: "not-a-url" }),
    /must be a valid http/,
  );

  const { agent: triage } = createAgentFromTemplate(auth.context, "support_triage");
  assert.throws(
    () => runAgent(auth.context, triage.id, { mailbox: "inbox", urgency_threshold: "extreme" }),
    /must be one of/,
  );
});
