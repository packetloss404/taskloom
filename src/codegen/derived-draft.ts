...
function deriveAppName(byPath: Map<string, GeneratedFile>, userGoal: string): string {
  ...
  if (pkg) {
    try {
      const parsed = JSON.parse(pkg.contents) as { name?: unknown };
  ...
  if (appFile) {
    const fromComponent = extractAppComponentName(appFile.contents);
...
function deriveApiRouteStubs(files: GeneratedFile[]): ApiRouteStub[] {
  return files
    ...
    .map((file) => {
      ...
      const method = inferApiMethod(file.contents);
      return {
        method,
        path,
        access: "private",
