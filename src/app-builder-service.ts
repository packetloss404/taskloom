...
async function tryFileTreeCodegen(
  ...
  try {
    const authorOptions: { preset?: ModelRoutingPresetId; workspaceId?: string; signal?: AbortSignal } = {};
    if (options.preset) authorOptions.preset = options.preset;
    if (options.workspaceId) authorOptions.workspaceId = options.workspaceId;
    if (options.signal) authorOptions.signal = options.signal;
    const result = await authorAppViaLLM(prompt, authorOptions, emit);
    if (!result) return null;

    const validateOptions: { workspaceId?: string; signal?: AbortSignal } = {};
    if (options.workspaceId) validateOptions.workspaceId = options.workspaceId;
    ...
    if (!validation.ok && validation.source === "real" && validation.errors.length > 0) {
      out.validationErrors = validation.errors;
    }
    return out;
  } catch (error) {
